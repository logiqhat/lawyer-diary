const { ok, notFound, badRequest, doc, sanitizeDate, buildDateUpdate, ensureCaseExists, log, getUserId, encryptDateFields, decryptDateFields, encryptUpdateValues } = require('/opt/nodejs/shared');
...
exports.handler = async (event) => {
  try {
    ...
    if (method === 'POST' && path === '/dates') {
      ...
      let item = sanitizeDate({ ...body, userId });
    ...
    if (method === 'GET' && path === '/dates') {
      ...
      items = await Promise.all(items.map((i) => decryptDateFields(i, userId)));
    ...
    if (method === 'GET' && path.startsWith('/dates/')) {
      ...
      const dec = await decryptDateFields(res.Item, userId);
    ...
    if (method === 'PUT' && path.startsWith('/dates/')) {
      ...
      let up = buildDateUpdate(body);
      // Only 'notes' is sensitive; do not encrypt keys used for indexing/queries
      ...
      const dec = await decryptDateFields(res.Attributes, userId);
